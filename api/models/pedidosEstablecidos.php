...
*/
class PedidosEstablecidos extends Validator
{
    ...
    */
    ...
    //Función para el reporte de los pedidos entre x fechas
    public function reportPEFX($fechai, $fechaf)
    {
        $sql = 'SELECT pes.id_pedidos_establecidos, pes.fecha_pedidoesta, pes.descripcionlugar_entrega, pes.montototal_pedidoesta, clt.nombre_cliente, clt.apellido_cliente, 
                clt.direccion_cliente, clt.correo_cliente,est.estado
                FROM pedidos_establecidos as pes
                INNER JOIN clientes AS clt ON pes.fk_id_cliente = clt.id_cliente
                INNER JOIN estados AS est ON pes.fk_id_estado = est.id_estados
                WHERE pes.fk_id_estado != 10 AND (pes.fecha_pedidoesta>= ? AND pes.fecha_pedidoesta<= ?) 
                ORDER BY pes.fecha_pedidoesta DESC';
        $params = array($fechai, $fechaf);
        return Database::getRows($sql, $params);
    }

    //Función para obtener el monto maximo, minimo y promedio de ventas entre x fechas
    public function reportPEMxMiPrFX($fechai, $fechaf)
    {
        $sql = 'SELECT MAX(montototal_pedidoesta) as maximo, MIN(montototal_pedidoesta) as minimo , AVG(montototal_pedidoesta)::numeric(10,2) as promedio, SUM(montototal_pedidoesta)::numeric(10,2) as suma 
                FROM pedidos_establecidos 
                WHERE fk_id_estado != 10 AND (fecha_pedidoesta>= ? AND fecha_pedidoesta<= ?)';
        $params = array($fechai, $fechaf);
        return Database::getRows($sql, $params);
    }

    //Función para obtener el número de pedidos durante esa fecha
    public function numeroPedidos($fecha)
    {
        $sql = 'SELECT COUNT(fecha_pedidoesta) as numero 
                FROM pedidos_establecidos 
                WHERE  fk_id_estado != 10 AND fecha_pedidoesta = ?';
        $params = array($fecha);
        return Database::getRows($sql, $params);
    }
}
