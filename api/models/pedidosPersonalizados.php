...
*/
class PedidosPersonalizados extends Validator
{
    ...
    */
    ...
    public function searchPedidoEnt($value)
    {
        ...
        FROM pedidos_personalizados as pes
        INNER JOIN clientes AS clt ON pes.fk_id_cliente = clt.id_cliente
        WHERE pes.fk_id_estado=6 and (clt.nombre_cliente ILIKE ? OR clt.apellido_cliente ILIKE ? OR cast(pes.id_pedidos_personalizado as varchar)ILIKE ? OR cast(pes.fecha_pedidopersonal as varchar) ILIKE ?) 
        ORDER BY pes.id_pedidos_personalizado DESC';
        $params = array("%$value%", "%$value%", "%$value%", "%$value%");
        return Database::getRows($sql, $params);
    }
    ...
    public function readPedido()
    {
        ...
        clt.nombre_cliente, clt.apellido_cliente, clt.direccion_cliente, clt.correo_cliente, tam.tamano
        FROM pedidos_personalizados as pes
        INNER JOIN clientes AS clt ON pes.fk_id_cliente = clt.id_cliente
    ...
    public function limitEntregado($limit)
    {
        ...
        INNER JOIN clientes AS clt ON pes.fk_id_cliente = clt.id_cliente
        INNER JOIN tamanos AS tm ON pes.fk_id_tamano = tm.id_tamanos
        WHERE pes.id_pedidos_personalizado NOT IN(SELECT id_pedidos_personalizado FROM pedidos_personalizados LIMIT ?) AND pes.fk_id_estado = 6
        ORDER BY pes.id_pedidos_personalizado DESC LIMIT 8;';
        $params = array($limit);
        return Database::getRows($sql, $params);
    }
    ...
    //Obtener los pedidos dentro de una fecha especifica
    public function reportPedidosFX($fechai, $fechaf)
    {
        $sql='SELECT pes.id_pedidos_personalizado, pes.fecha_pedidopersonal, pes.descripcion_pedidopersonal, pes.imagenejemplo_pedidopersonal, 
            clt.nombre_cliente, clt.apellido_cliente, tm.tamano,est.estado
            FROM pedidos_personalizados as pes
            INNER JOIN clientes AS clt ON pes.fk_id_cliente = clt.id_cliente
            INNER JOIN tamanos AS tm ON pes.fk_id_tamano = tm.id_tamanos
            INNER JOIN estados AS est ON pes.fk_id_estado = est.id_estados
            WHERE pes.fk_id_estado != 10 AND (pes.fecha_pedidopersonal>= ? AND pes.fecha_pedidopersonal<= ?) 
            ORDER BY pes.id_pedidos_personalizado DESC';
