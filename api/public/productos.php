...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        case 'readTOP3':
            if ($result['dataset'] = $producto->top3Productos()) {
                $result['status'] = 1;
            ...
            break;
        case 'readAllLimit':
            if ($result['dataset'] = $producto->readAllProductsLP($_POST['limit'])) {
                $result['status'] = 1;
            } elseif (Database::getException()) {
                $result['exception'] = Database::getException();
            } else {
            ...
            break;
        case 'readAllLimitNT':
            if ($result['dataset'] = $producto->readAllProductsLPNT($_POST['limit'])) {
                $result['status'] = 1;
            } elseif (Database::getException()) {
                $result['exception'] = Database::getException();
            } else {
        ...
        case 'searchFilter':
            ...
            if ($result['dataset'] = $producto->readAllProductsLFilt($_POST['search'], $_POST['cat'])) {
                $result['status'] = 1;
            } elseif (Database::getException()) {
                $result['exception'] = Database::getException();
            } else {
