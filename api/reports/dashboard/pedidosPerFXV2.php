...
if (isset($_GET['fechai']) && isset($_GET['fechaf'])) {
    ...
    if ($pedido->setFecha($_GET['fechai']) && $pedido->setFecha($_GET['fechaf'])) {
        ...
        if ($dataPedido = $pedido->reportPedidosFX($_GET['fechai'],$_GET['fechaf'])) {
            ...
            $pdf->cell(25, 10, utf8_decode('Codigo Pedidos'), 1, 0, 'C', 1);
            ...
            foreach ($dataPedido as $rowPedido) {
                $pdf->SetWidths(array(25,75,40,50,25,25,30));//Seteamos el ancho de las celdas
                $pdf->setHeight(15);
                $pdf->Row(array('6',utf8_decode('Quiero un amigurumi parecido a mi rostro sise puede'), $pdf->Image("../../images/pedidosper/".$rowPedido['imagenejemplo_pedidopersonal'], $pdf->GetX()+100, $pdf->GetY(),40,30),utf8_decode('Jesús Gerardo Esquivel Ramírez'),utf8_decode('Pequeño:12cm o menos'),utf8_decode('Pendiente'),utf8_decode('2022-02-24')));
            ...
            $pdf->cell(0, 10, utf8_decode('No hay pedidos personalizados entre estas fechas'), 1, 1);
        }
